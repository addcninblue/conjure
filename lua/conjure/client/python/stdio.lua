...
config.merge({client = {python = {stdio = {mapping = {start = "cs", stop = "cS", interrupt = "ei"}, command = "ipython", prompt_pattern = ">>>"}}}})
...
local function form_node_3f(node)
  return not string.match(ts["node->str"](node), "julia>")
end
...
local function doc_str(opts)
  return log.append((comment_prefix .. "doc-str is not implemented for Python"))
end
...
local function start()
  if state("repl") then
    ...
    local function _10_()
      ...
      --[[ (with-repl-or-warn (fn [repl] (repl.send (prep-code "using REPL") (fn [msgs] (log.append (-> msgs unbatch format-msg))) {:batch? true}))) ]]--
      return nil
    end
